...
void project2() {
	...
	cout << "BoxTree construction time: " << end - start << "ms." << endl;
	scn.AddObject(tree);
	...
	// Create camera
	Camera cam;
	cam.LookAt(Vector3(-0.1f, 0.1f, 0.2f), Vector3(-0.05f, 0.12f, 0.0f), Vector3(0.0f, 1.0f, 0.0f));
	cam.SetFOV(40.0f);
	cam.SetAspect(1.33f);
	cam.SetResolution(800, 600);
	...
	cout << "Render time: " << end - start << "ms." << endl;
...
void project3() {
	...
	// Load dragon mesh 
	MeshObject dragon;
	dragon.LoadPLY("dragon.ply", NULL);
